            ...
            >
                <Stack width="full" gap="5">
                    ...
                    <Table.Root size="sm" variant="outline" striped>
                        <Table.Header >
                            <Table.Row colorPalette={"gray"} bg={"#7DA9AE"}>
                                <Table.ColumnHeader >Product</Table.ColumnHeader>
                                <Table.ColumnHeader >Category</Table.ColumnHeader>
                                <Table.ColumnHeader textAlign="end">Price</Table.ColumnHeader>
                            </Table.Row>
                        </Table.Header>
                        <Table.Body bg={bgColor}>
                            {items.map((item) => (
                                <Table.Row bg={bgColor} key={item.id}>
                                    <Table.Cell bg={bgColor}>{item.name}</Table.Cell>
                                    <Table.Cell bg={bgColor}>{item.category}</Table.Cell>
                                    <Table.Cell bg={bgColor} textAlign="end">{item.price}</Table.Cell>
                                </Table.Row>
                            ))}
                        </Table.Body>
                    </Table.Root>
                    ...
                    <Pagination.Root count={items.length * 5} pageSize={5} page={1}>
                        <ButtonGroup color={bgColor} variant="plain" size="sm" wrap="wrap">
                            ...
                            <Pagination.Items
                                render={(page) => (
                                    <IconButton color={page.isSelected ? bgColor : "gray"} variant={{ base: "ghost", _selected: "surface" }}>
                                        {page.value}
                                    </IconButton>
                                )}
                            />
